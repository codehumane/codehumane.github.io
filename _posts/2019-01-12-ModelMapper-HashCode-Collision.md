...
기존 에러 로그만으로는 충분히 문제를 찾기 어려워, 에러 발생시 문제라고 예상되는 지점을 추가로 로깅함. 바로, [`TypeMapStore`](https://github.com/modelmapper/modelmapper/blob/master/core/src/main/java/org/modelmapper/internal/TypeMapStore.java)의 `typeMaps` 필드. 그랬더니, 처음 예상했던 문제가 원인임을 발견. 캐시에 잘못 매핑된 데이터가 저장되어 있는 것.
...
1. 위에서 작성한 코드에 테스트 코드를 더하여 PR 제출.
2. [https://github.com/modelmapper/modelmapper/pull/438](https://github.com/modelmapper/modelmapper/pull/438)
3. 그리고 머지 됨.
